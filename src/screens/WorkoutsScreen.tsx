...
export function WorkoutsScreen(){
  const navigate=useNavigate(); const [selected,setSelected]=useState('B'); const workout=workouts.find(w=>w.id===selected) ?? workouts[1];
  return <AppShell title="Treinos"><div className="page-pad">
    <section className="hero-card user-summary"><BrandLogo compact/><div className="hero-card__divider"/><div><h2>Hallan Fernando Lehrbach</h2><span className="status-chip status-chip--success"><CheckCircle2 size={15}/> Ativo</span><p><Clock3 size={15}/> Última atualização hoje</p></div></section>
