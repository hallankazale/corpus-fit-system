...
export function ActiveWorkoutScreen() {
  ...
  return <AppShell title="Treino em andamento"><div className="page-pad active-workout">
    ...
    <section className="active-hero"><div className="exercise-photo-placeholder"><Dumbbell size={72} /><span>{exercise.name}</span></div><div><h1>{exercise.name}</h1><p>Grupo muscular</p><div className="muscle-tags"><span>{exercise.muscle_group || "Geral"}</span></div><div className="active-meta"><span><Dumbbell /><small>Séries x reps</small><b>{exercise.sets} x {exercise.reps_min}{exercise.reps_max !== exercise.reps_min ? `-${exercise.reps_max}` : ""}</b></span><span><Weight /><small>Carga sugerida</small><b>{exercise.suggested_load_kg == null ? "Livre" : `${exercise.suggested_load_kg} kg`}</b></span><span><Clock3 /><small>Descanso</small><b>{exercise.rest_seconds} s</b></span></div>{exercise.notes && <div className="info-strip active-note">{exercise.notes}</div>}</div></section>
