...
async function split2x2(dataUrl: string): Promise<Frame[]> {
  ...
  const halfW = Math.floor(img.naturalWidth / 2), halfH = Math.floor(img.naturalHeight / 2);
  const cells = [[0,0],[halfW,0],[0,halfH],[halfW,halfH]];
  return cells.map(([sx, sy], index) => {
    const canvas = document.createElement("canvas"); canvas.width = halfW; canvas.height = halfH;
    const ctx = canvas.getContext("2d")!; ctx.fillStyle = "#fff"; ctx.fillRect(0,0,halfW,halfH);
...
async function shareBytes(bytes: Uint8Array, filename: string, mime: string) {
...
export function GifMakerApp() {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => setPreviewIndex(i => (i + 1) % playback.length), delay);
    return () => clearTimeout(timer);
  }, [previewIndex, delay, playback]);

  async function onCollage(e: ChangeEvent<HTMLInputElement>) {
    const file = e.target.files?.[0]; if (!file) return;
    setWorking(true);
    try { const data = await fileToDataUrl(file); setFrames(await split2x2(data)); setMessage("Colagem dividida em 4 quadros. Confira a ordem e gere o GIF."); }
    finally { setWorking(false); e.target.value = ""; }
  }
  ...
  async function onFrames(e: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      setFrames(next); setMessage(`${next.length} quadros importados.`);
    } finally { setWorking(false); e.target.value = ""; }
  }

  function move(index: number, direction: -1 | 1) {
    const target = index + direction; if (target < 0 || target >= frames.length) return;
    setFrames(current => { const copy = [...current]; [copy[index], copy[target]] = [copy[target], copy[index]]; return copy; });
  }

  async function generate() {
    if (frames.length < 2) { setMessage("Adicione pelo menos 2 quadros."); return; }
    ...
    try {
      ...
      await saveGif(item); setSaved(await listSaved());
      setMessage(`Pronto: ${slug}.gif • ${(bytes.length / 1024 / 1024).toFixed(2)} MB`);
      await shareBytes(bytes, `${slug}.gif`, "image/gif");
    } catch (error) { setMessage(`Erro ao gerar: ${error instanceof Error ? error.message : String(error)}`); }
    finally { setWorking(false); }
  }

  async function exportZip() {
    if (!saved.length) { setMessage("Gere pelo menos um GIF antes de exportar o pacote."); return; }
    setWorking(true);
    try {
      ...
      await shareBytes(zip, "projeto-trincado-gifs.zip", "application/zip");
      setMessage(`Pacote organizado com ${saved.length} GIF(s) pronto para compartilhar.`);
    } catch (error) { setMessage(`Erro no pacote: ${error instanceof Error ? error.message : String(error)}`); }
    finally { setWorking(false); }
  }

  return <div className="gm-app">
    <header className="gm-header"><div><small>PROJETO TRINCADO</small><h1>GIF Maker</h1><p>Crie nossos exercícios no celular e exporte tudo organizado.</p></div><span>v0.1</span></header>
    ...
    <main>
      ...
      <section className="gm-card">
        ...
        <div className="gm-actions">
          <button onClick={() => collageInput.current?.click()}>Importar colagem 2×2</button>
          <button className="secondary" onClick={() => framesInput.current?.click()}>Importar quadros</button>
        </div>
      ...
      <section className="gm-card">
        <h2>2. Ajustes</h2>
        <div className="gm-grid"><label>Velocidade<select value={delay} onChange={e => setDelay(Number(e.target.value))}><option value="260">Lenta</option><option value="180">Média</option><option value="120">Rápida</option></select></label><label>Resolução<select value={size} onChange={e => setSize(Number(e.target.value))}><option value="384">384×384 leve</option><option value="512">512×512 recomendada</option><option value="640">640×640 alta</option></select></label></div>
        ...
        <small className="muted">O app faz ida e volta automaticamente para o loop não dar salto.</small>
      </section>

      <section className="gm-card">
        <h2>3. Gerar</h2><button className="primary" disabled={working || frames.length < 2} onClick={generate}>{working ? "Processando..." : "Gerar GIF e compartilhar"}</button><p className="gm-message">{message}</p>
      </section>
      ...
      <section className="gm-card">
        ...
        {saved.length === 0 ? <div className="empty">Nenhum GIF gerado ainda.</div> : <div className="gm-saved">{saved.sort((a,b)=>b.createdAt.localeCompare(a.createdAt)).map(item => <article key={item.id}><div><strong>{item.name}</strong><small>{item.category}/{item.slug}/{item.slug}.gif • {(item.bytes.length/1024/1024).toFixed(2)} MB</small></div><div><button onClick={() => shareBytes(item.bytes, `${item.slug}.gif`, "image/gif")}>Compartilhar</button><button className="danger" onClick={async()=>{await deleteGif(item.id);setSaved(await listSaved())}}>Apagar</button></div></article>)}</div>}
